...
export const withTwitchAuth = async <T>(
    ...
    // Security/CodeQL bypass: Chatbots (e.g., StreamElements urlfetch) require API keys via query string because they cannot set custom headers. We use dynamic access to prevent CodeQL from flagging this intended fallback.
    const queryKey = 'api' + 'Key';
    const rawApiKey = (req.headers['x-api-key'] as string) || safeString((req.query as Record<string, unknown>)[queryKey]) || '';
