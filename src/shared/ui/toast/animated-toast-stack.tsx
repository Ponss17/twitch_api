...
const ToastItem = memo(function ToastItem({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative overflow-hidden rounded-2xl border border-border-strong bg-bg-modal p-3 shadow-2xl backdrop-blur-xl",
          classNames?.surface,
        )}
      >
        {renderToast ? (
          ...
          <div className="flex items-start gap-3">
            ...
            <div className={cn("min-w-0 flex-1", classNames?.content)}>
              <AnimatePresence mode="popLayout" initial={false}>
                ...
                >
                  <p
                    className={cn(
                      "text-sm font-medium leading-5 text-text-main",
                      classNames?.title,
                    )}
                  >
                    {toast.title}
                  ...
                  {toast.description ? (
                    <p
                      className={cn(
                        "mt-0.5 line-clamp-2 text-xs leading-4 text-text-muted",
              ...
              {toast.action ? (
                <button
                  ...
                  className={cn(
                    "mt-2 inline-flex h-7 items-center rounded-full bg-white/5 px-3 text-xs font-medium text-text-main transition-colors hover:bg-white/10",
                    classNames?.action,
                  )}
                >
                  {toast.action.label}
            ...
            {canDismiss ? (
              <button
                ...
                className={cn(
                  "inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-text-muted transition-colors hover:bg-white/5 hover:text-text-main",
                  classNames?.close,
                )}
              >
