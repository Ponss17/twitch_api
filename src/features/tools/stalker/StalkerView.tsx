...
import { Play, Pause, Users, Radio, Search, RotateCw, User, Eye } from 'lucide-react';
...
export function StalkerView({ active = true }: { active?: boolean }) {
    ...
    const loadChatters = useCallback(
            ...
            const run = (async () => {
                ...
                try {
                    const data = await apiFetch<StalkerUser[] | { chatters?: (string | StalkerUser)[] }>(
                        `${API_ENDPOINTS.CHATTERS}?channel=${encodeURIComponent(login)}&source=stalker&annotate=0`,
                        session
                    );
                    const list = Array.from(Array.isArray(data) ? data : data.chatters ?? []);
        ...
        [session, showToast, stalker.toasts.errorLoad]
    );
