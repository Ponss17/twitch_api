...
function DetailRow({ icon: Icon, label, value, highlight = false, isLast = false }: { icon: React.ElementType; label: string; value: React.ReactNode; highlight?: boolean; isLast?: boolean }) {
    return (
        <div className={`flex items-center justify-between py-3 ${isLast ? '' : 'border-b border-border-subtle'}`}>
            <div className="flex items-center gap-3">
                <div className={`flex h-7 w-7 items-center justify-center rounded-lg bg-text-main/5 text-text-muted`}>
        ...
        >
            <div className="flex flex-col gap-6 pt-1 pb-6">
                ...
                <div className="flex flex-col items-center justify-center py-4 text-center">
                    <div className={`mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-border-strong bg-text-main/5 shadow-lg`}>
                ...
                {/* Flat List Card */}
                <div className="rounded-xl border border-border-subtle bg-text-main/5 px-4 py-1">
                ...
                <div className="flex flex-col gap-2 pt-2">
                    ...
                    <div className="relative overflow-hidden rounded-xl border border-border-subtle bg-bg-secondary">
                        {/* Editor Header Dots */}
                        <div className="flex items-center gap-1.5 border-b border-border-subtle bg-text-main/5 px-4 py-2.5">
                        ...
                        <div className="flex">
                            <div className="flex flex-col items-end border-r border-border-subtle bg-text-main/5 px-3 py-4 text-[0.65rem] text-text-muted font-mono select-none">
                                {jsonStr.split('\n').map((_, i) => (
                                    <span key={i} className="leading-[1.4rem]">{i + 1}</span>
                                ))}
                            </div>
                            <pre className="overflow-x-auto p-4 text-[0.75rem] font-mono leading-[1.4rem] text-text-muted [scrollbar-width:thin]">
                                {jsonStr.split('\n').map((line, i) => {
