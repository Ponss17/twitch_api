...
const PLACEHOLDER_CLASS = 'text-2xl font-extrabold leading-none tracking-tight text-white/35';
...
export function HomeHero({
    ...
    const latencyBootstrappedRef = useRef(false);
    const latencyWasLoadingRef = useRef(isLoading);

    useLayoutEffect(() => {
        if (isLoading) {
            latencyWasLoadingRef.current = true;
            return;
        }
        ...
        const isFirstReveal = latencyWasLoadingRef.current || !latencyBootstrappedRef.current;
        const start = isFirstReveal ? 0 : prevLatencyRef.current;
        animateValue(el, start, latencyMs, 1500, suffix);
        prevLatencyRef.current = latencyMs;
        latencyBootstrappedRef.current = true;
        latencyWasLoadingRef.current = false;
    }, [latencyMs, isLoading]);
                    ...
                    <div className={STATS_ROW}>
                        ...
                        <div className={H_STAT}>
                            ...
                            <div className="flex min-w-0 items-center gap-2">
                                ...
                                {isLoading ? (
                                    <span className={PLACEHOLDER_CLASS}>—</span>
                                ) : (
                                    <span
                                        ref={latencyRef}
                                        className="text-2xl font-extrabold leading-none tracking-tight text-white"
