...
SET search_path = public
AS $$
  SELECT u.discord_id::text, u.login::text,
    COALESCE(u.display_name, u.login)::text, u.discord_linked_at
  FROM public.users u
...
ALTER TABLE public.discord_link_events DROP CONSTRAINT IF EXISTS discord_link_events_event_check;
ALTER TABLE public.discord_link_events ADD CONSTRAINT discord_link_events_event_check
...
BEGIN
  IF TG_OP = 'UPDATE' AND OLD.discord_id IS DISTINCT FROM NEW.discord_id THEN
    IF OLD.discord_id IS NOT NULL THEN
      ...
      VALUES ('unlinked', OLD.discord_id, OLD.login, COALESCE(OLD.display_name, OLD.login));
    END IF;
    IF NEW.discord_id IS NOT NULL THEN
      INSERT INTO public.discord_link_events (event, discord_id, login, display_name)
      VALUES ('linked', NEW.discord_id, NEW.login, COALESCE(NEW.display_name, NEW.login));
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER users_discord_link_events
  ...
  FOR EACH ROW EXECUTE FUNCTION public.notify_discord_link_change();
