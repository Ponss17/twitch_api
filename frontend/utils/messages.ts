export const Messages = {
    ...
    Common: {
        ...
        error: (msg: string) => `<div class="error-msg"><i class="fa-solid fa-triangle-exclamation"></i> ${msg}</div>`,
    ...
    Tracker: {
        ...
        timeUp: '¡TIEMPO!',
        started: (min: number) => `<i class="fa-solid fa-hourglass-start fa-spin"></i>  Tracker iniciado (${min} min)`,
        ...
        winner: (word: string, count: number) => `👑 Ganador: <strong>"${word}"</strong> <span style="font-size:0.9em; opacity:0.8">(${count})</span>`,
    ...
    Commands: {
        ...
        success: (text: string) => `<span class="text-success"><i class="fa-solid fa-check"></i> ${text}</span>`,
        error: (text: string) => `<span class="text-danger"><i class="fa-solid fa-triangle-exclamation"></i> Error: ${text}</span>`
    },
    Analytics: {
    ...
    Details: {
        ...
        affiliate: 'Afiliado',
        user: 'Usuario',
        created: (date: any) => `Cuenta creada el: ${new Date(date).toLocaleDateString()}`,
        new: 'Nueva',
    ...
    ChatSim: {
        ...
        btnText: "Chat",
        followage: (user: string, channel: string, time: string) => `@${user} sigue a @${channel} desde hace ${time}.`,
        clip: (user: string, url: string) => `🎬 Clip creado por <span style="color:#FF69B4">@${user}</span>: ${url}`,
        shoutout: (user: string, game: string) => `¡Vayan a seguir a <span style="color:#bf94ff">@${user}</span>! Estaba jugando ${game}`
    }
};
