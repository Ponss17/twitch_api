var g=Object.defineProperty;var p=(e,t)=>g(e,"name",{value:t,configurable:!0});import{UI as o}from"../../core/ui.js";import{Messages as E}from"../../shared/i18n/messages.js";import{ClipsMessages as m}from"./clips/messages.js";import{AuthMessages as v}from"../../shared/i18n/authMessages.js";import{DASHBOARD_CONFIG as C}from"./dashboard-config.js";const{API_ENDPOINTS:b}=C;import{cache as h,CACHE_TTL as T}from"../../services/cacheService.js";const B={session:null,initialized:!1,allClips:[],currentClips:[],favorites:[],observer:null,currentPage:1,ITEMS_PER_PAGE:20,cssLoaded:!1,init(e){this.session=e,this.cssLoaded||(import("../../shared/utils/loader.js").then(({Loader:t})=>{t.loadCSS("css/sections/clips.css")}),this.cssLoaded=!0),this.loadFavorites(),this.observer||(this.observer=new IntersectionObserver((t,i)=>{t.forEach(s=>{if(s.isIntersecting){const r=s.target;r.src=r.dataset.src,r.classList.remove("lazy-img"),i.unobserve(r)}})},{rootMargin:"50px"})),this.setupUI(),this.initialized=!0},activate(){this.allClips.length===0&&this.loadClips()},deactivate(){this.observer&&this.observer.disconnect()},loadFavorites(){if(this.session)try{const e=localStorage.getItem(`clips_favs_${this.session.userId}`);this.favorites=e?JSON.parse(e):[]}catch(e){console.error("Error loading favorites",e),this.favorites=[]}},saveFavorites(){if(this.session)try{localStorage.setItem(`clips_favs_${this.session.userId}`,JSON.stringify(this.favorites))}catch(e){console.error("Error saving favorites",e)}},toggleFavorite(e){this.favorites.includes(e)?(this.favorites=this.favorites.filter(t=>t!==e),o.showToast("Clip eliminado de favoritos","info")):(this.favorites.push(e),o.showToast("Clip a\xF1adido a favoritos","success")),this.saveFavorites(),this.updateFavoriteBtn(e)},setupUI(){this.setupFilters();const e=document.getElementById("refresh-clips-btn");e&&e.addEventListener("click",()=>{this.loadClips(!0)})},setupFilters(){const e=document.getElementById("clips-search"),t=document.getElementById("clips-sort");e&&e.addEventListener("input",this.debounce(()=>{this.filterAndRender()},300)),t&&t.addEventListener("change",()=>{this.filterAndRender()})},debounce(e,t){let i;return(...s)=>{const r=p(()=>{clearTimeout(i),e(...s)},"later");clearTimeout(i),i=setTimeout(r,t)}},async loadClips(e=!1){const t=document.getElementById("clips-gallery");if(!t||!this.session)return;const i=`clips_${this.session.userId}`;if(!e){const s=h.get(i);if(s){this.allClips=s,this.filterAndRender();return}}this.renderSkeleton(t),e&&o.showToast("Actualizando clips...","info");try{const{apiKey:s,token:r,login:a}=this.session,n={"Content-Type":"application/json"};let l=b.CLIPS;r?(n.Authorization=`Bearer ${r}`,l+=`?channel=${a}`):s&&(l+=`?apiKey=${s}&channel=${a}`);const c=await fetch(l,{headers:n});if(!c.ok){const f=c.status===401||c.status===403;throw new Error(f?"auth_error":"fetch_error")}const d=await c.json(),u=Array.isArray(d)?d:d.clips||d.data||[];this.allClips=u,u.length>0&&h.set(i,u,T),this.filterAndRender()}catch(s){this.handleError(s,t)}},handleError(e,t){const i=e.message==="auth_error";if(o.showToast(i?v.expired:m.loadError,"error"),i)t.innerHTML=`
            ...
            `,document.getElementById("relogin-clips-btn")?.addEventListener("click",()=>{import("../../core/auth.js").then(s=>s.Auth.relogin())});else{t.innerHTML=E.Common.error(e.message);const s=document.getElementById("retry-clips-btn");s&&(s.onclick=()=>this.loadClips())}},renderSkeleton(e){e.innerHTML=Array(8).fill(0).map(()=>`
