import{fetchAdmin as i,logout as g,checkAuth as p}from"./auth.js";let l=null,v="overview";window.switchSection=e=>{v=e,document.querySelectorAll(".admin-section").forEach(r=>r.classList.remove("active")),document.querySelectorAll(".nav-item").forEach(r=>r.classList.remove("active")),document.getElementById(`section-${e}`)?.classList.add("active");const s=document.querySelector(`button[onclick*="switchSection('${e}')"]`);s&&s.classList.add("active");const t={overview:"Resumen General",users:"Gesti\xF3n de Usuarios",system:"Salud del Sistema",limits:"L\xEDmites & Tr\xE1fico",logs:"Logs del Sistema",security:"Seguridad & Admins",config:"Configuraci\xF3n"},a=document.getElementById("current-section-title");a&&(a.textContent=t[e]||"Panel Admin"),e==="overview"?(b(),c()):e==="users"?c():e==="system"?E():e==="config"?h():e==="security"?d():e==="logs"&&window.refreshLogs()},window.refreshLogs=async()=>{const e=document.getElementById("logs-list");if(e)try{const t=await(await i("/api/twitch/admin/logs")).json();if(!t.length){e.innerHTML='<div class="loading-cell">No hay logs registrados todav\xEDa.</div>';return}e.innerHTML=t.map(a=>`
        ...
        `).join(""),e.scrollTop=0}catch{o("Error","Error al cargar logs","error")}};const y=e=>{const s=document.getElementById("usersChart");if(!s)return;l&&l.destroy();const t=e.map(r=>r.displayName),a=e.map(r=>r.totalRequests||0);l=new Chart(s,{type:"bar",data:{labels:t,datasets:[{label:"Total Requests",data:a,backgroundColor:"rgba(145, 70, 255, 0.5)",borderColor:"rgba(145, 70, 255, 1)",borderWidth:1}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{y:{beginAtZero:!0,grid:{color:"rgba(255, 255, 255, 0.1)"},ticks:{color:"#efeff1"}},x:{grid:{display:!1},ticks:{color:"#efeff1"}}},plugins:{legend:{labels:{color:"#efeff1"}}}}})},u=e=>{const s=document.getElementById("users-table-body");if(s){if(e.length===0){s.innerHTML='<tr><td colspan="6" class="loading-cell">No se encontraron usuarios.</td></tr>';return}s.innerHTML=e.map(t=>`
    ...
    `).join("")}},f=()=>{let e=document.getElementById("toast-container");return e||(e=document.createElement("div"),e.id="toast-container",e.className="toast-container",document.body.appendChild(e)),e},o=(e,s,t="info")=>{const a=f(),r=document.createElement("div");r.className=`toast ${t}`;const n=t==="success"?"fa-circle-check":t==="error"?"fa-circle-exclamation":"fa-circle-info";r.innerHTML=`
        <i class="fa-solid ${n} toast-icon"></i>
        <div class="toast-content">
            <span class="toast-title">${e}</span>
            <span class="toast-message">${s}</span>
        </div>
    `,a.appendChild(r),requestAnimationFrame(()=>{r.classList.add("show")}),setTimeout(()=>{r.classList.remove("show"),setTimeout(()=>r.remove(),300)},4e3)};let m=[];const w=()=>{const e=document.getElementById("user-search");e&&e.addEventListener("input",s=>{const t=s.target.value.toLowerCase(),a=m.filter(r=>r.displayName.toLowerCase().includes(t)||r.login.toLowerCase().includes(t)||r.userId.includes(t));u(a)})},c=async()=>{try{const e=await i("/api/twitch/admin/users");if(!e.ok)throw new Error(`Failed to load users: ${e.status}`);const s=await e.json();m=s,u(s),y(s)}catch(e){console.error("Error in loadUsers:",e),o("Error","Error cargando usuarios","error")}},b=async()=>{try{const e=await i("/api/twitch/admin/stats/global");if(!e.ok)throw new Error("Failed to load global stats");const s=await e.json(),t=document.getElementById("kpi-total-users"),a=document.getElementById("kpi-total-requests"),r=document.getElementById("kpi-active-users");t&&(t.innerText=s.totalUsers),a&&(a.innerText=s.totalRequests),r&&(r.innerText=s.activeUsers)}catch(e){console.error("Error loadGlobalStats:",e)}},E=async()=>{const e=document.getElementById("system-status-container");if(e)try{const s=await i("/api/twitch/admin/system/status");if(!s.ok)throw new Error("Failed to load system status");const t=await s.json();e.innerHTML=Object.entries(t.services).map(([a,r])=>`
        ...
        `).join("")}catch{e.innerHTML='<div class="error-msg" style="display:block">Error cargando estado del sistema</div>'}},h=async()=>{const e=document.getElementById("config-list-container");if(e)try{const s=[{key:"NODE_ENV",value:"production"},{key:"PORT",value:"3000"},{key:"TWITCH_CLIENT_ID",value:"********"},{key:"GROQ_API_KEY",value:"Suministrada \u2705"},{key:"ADMIN_ENABLED",value:"true"}];e.innerHTML=s.map(t=>`
            <div class="config-item">
                <span class="config-key">${t.key}</span>
                <span class="config-value">${t.value}</span>
            </div>
        `).join("")}catch{e.innerHTML='<div class="error-msg" style="display:block">Error cargando configuraci\xF3n</div>'}};window.blockUser=async e=>{const s=prompt("\xBFRaz\xF3n del bloqueo?");if(s!==null)try{(await i(`/api/twitch/admin/users/${e}/status`,{method:"POST",body:JSON.stringify({isActive:!1,reason:s})})).ok?(c(),o("\xC9xito","Usuario bloqueado correctamente","success")):o("Error","No se pudo bloquear al usuario","error")}catch(t){console.error(t),o("Error","Error de conexi\xF3n","error")}},window.unblockUser=async e=>{if(confirm("\xBFEst\xE1s seguro de desbloquear a este usuario?"))try{(await i(`/api/twitch/admin/users/${e}/status`,{method:"POST",body:JSON.stringify({isActive:!0})})).ok?(c(),o("\xC9xito","Usuario desbloqueado","success")):o("Error","No se pudo desbloquear al usuario","error")}catch(s){console.error(s),o("Error","Error de conexi\xF3n","error")}},window.resetKey=async e=>{if(confirm("\xBFGenerar nueva API Key? La anterior dejar\xE1 de funcionar."))try{(await i(`/api/twitch/admin/users/${e}/reset-key`,{method:"POST"})).ok?(c(),o("API Key Generada","El usuario tiene una nueva clave","success")):o("Error","No se pudo resetear la clave","error")}catch(s){console.error(s),o("Error","Error de conexi\xF3n","error")}},window.deleteUser=async e=>{if(confirm("\xBFELIMINAR usuario permanentemente? Esta acci\xF3n no se puede deshacer."))try{(await i(`/api/twitch/admin/users/${e}`,{method:"DELETE"})).ok?(c(),o("\xC9xito","Se ha borrado el usuario y sus datos","success")):o("Error","No se pudo eliminar al usuario","error")}catch(s){console.error(s),o("Error","Error de conexi\xF3n","error")}},window.updateRateLimit=async(e,s)=>{const t=prompt("Asignar nuevo l\xEDmite de peticiones (req/min):",s.toString());if(t===null)return;const a=parseInt(t);if(isNaN(a)||a<0){o("Error","El l\xEDmite debe ser un n\xFAmero v\xE1lido >= 0","error");return}try{const r=await i(`/api/twitch/admin/users/${e}/rate-limit`,{method:"POST",body:JSON.stringify({limit:a})});if(r.ok)c(),o("\xC9xito",`L\xEDmite actualizado a ${a} req/min`,"success");else{const n=await r.json();o("Error",n.error||"No se pudo actualizar el l\xEDmite","error")}}catch(r){console.error(r),o("Error","Error de conexi\xF3n","error")}},window.addAdminPrompt=async()=>{const e=prompt("Ingresa el Twitch ID num\xE9rico del nuevo administrador:");if(e)try{const s=await i("/api/twitch/admin/admins",{method:"POST",body:JSON.stringify({userId:e})});if(s.ok)o("\xC9xito","Administrador a\xF1adido correctamente","success"),d();else{const t=await s.json();o("Error",t.error||"No se pudo a\xF1adir al admin","error")}}catch{o("Error","Error de conexi\xF3n","error")}},window.removeAdmin=async e=>{if(confirm("\xBFQuitar permisos de administrador a este usuario?"))try{const s=await i(`/api/twitch/admin/admins/${e}`,{method:"DELETE"});if(s.ok)o("\xC9xito","Permisos revocados","success"),d();else{const t=await s.json();o("Error",t.error||"No se pudo quitar al admin","error")}}catch{o("Error","Error de conexi\xF3n","error")}};const d=async()=>{const e=document.getElementById("admins-table-body");if(e)try{const s=await i("/api/twitch/admin/admins");if(!s.ok)throw new Error("Failed to load admins");const{admins:t,rootId:a}=await s.json();if(t.length===0){e.innerHTML='<tr><td colspan="4" style="text-align: center; padding: 20px; color: var(--text-muted)">No hay administradores adicionales</td></tr>';return}e.innerHTML=t.map(r=>{const n=r.userId===a;return`
            <tr>
                <td>
                ...
                <td>
                    <span class="status-badge ${n?"active":"warn"}" style="font-size: 11px;">
                        ${n?"Root Admin":"Admin Delegado"}
                    </span>
                </td>
                <td>
                    ${n?'<small style="color: var(--text-muted)">Protegido</small>':`
