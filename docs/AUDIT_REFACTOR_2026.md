...
8. **Integración Definitiva de Vercel Speed Insights**: Tras una detección fallida con script tags manuales, se realizó una re-integración siguiendo el estándar oficial de Vercel para proyectos Vanilla. 
    - **Paquete Oficial**: Se re-instanció `injectSpeedInsights()` desde `@vercel/speed-insights` en los bundles de `app-dashboard.js` y `landing.js`.
    - **Middleware Bypass**: Se actualizó `routeHelpers.ts` para que el middleware de Express ignore cualquier petición que comience con `/_vercel`, asegurando que los reportes de vitals y la carga de scripts internos de Vercel no sean interceptados ni bloqueados por reglas de autenticación o CSRF.
    - **Limpieza de HTML**: Se eliminaron los script tags manuales para evitar duplicidades y asegurar que solo el paquete oficial gestione la recolección de métricas.
