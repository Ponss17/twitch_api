...
jest.mock('../../backend/src/features/minigames/magic8.service', () => ({
    __esModule: true,
    generateMagic8Response: jest.fn()
}));

jest.mock('../../backend/src/features/minigames/russian.service', () => ({
    playRussianRoulette: jest.fn()
}));

jest.mock('../../backend/src/features/minigames/duel.service', () => ({
    playDuel: jest.fn()
}));

jest.mock('../../backend/src/features/minigames/slots.service', () => ({
    playSlots: jest.fn()
}));

jest.mock('../../backend/src/features/integrations/nightbotResponse', () => ({
...
import { generateMagic8Response } from '../../backend/src/features/minigames/magic8.service';
...
import { playSlots as playSlotsService } from '../../backend/src/features/minigames/slots.service';
import {
...
import { askMagic8, playRussian, startDuel, playSlots } from '../../backend/src/features/minigames/minigames.controller';
...
describe('minigamesController', () => {
    ...
    describe('playSlots', () => {
        it('should spin slots and track activity', async () => {
            const req = mockReq({ query: { user: 'Player1' } });
            const res = mockRes();

            (playSlotsService as jest.Mock).mockReturnValue({
                message: '🎰 🍒 | 🍋 | 🔔 — @Player1 no se lleva nada esta vez. ¡Otra!',
                messages: ['a', 'b', 'c']
            });

            await playSlots(req, res);

            expect(playSlotsService).toHaveBeenCalledWith('Player1', 'es');
            expect(dbService.addUserActivity).toHaveBeenCalledWith(
                '123',
                expect.objectContaining({ type: 'slots', user: 'Player1' })
            );
            expect(res.send).toHaveBeenCalledWith(
                '🎰 🍒 | 🍋 | 🔔 — @Player1 no se lleva nada esta vez. ¡Otra!'
            );
            expect(scheduleNightbotFollowUps).not.toHaveBeenCalled();
        });
        ...
        it('should chain Nightbot messages via Response-Url', async () => {
            ...
            const req = mockReq({ query: { user: 'Player1', interval: '5' } });
            const res = mockRes();
            const messages = ['spin1', 'spin2', 'final'];
            ...
            expect(res.send).toHaveBeenCalledWith('spin1');
            expect(scheduleNightbotFollowUps).toHaveBeenCalledWith(
                responseUrl,
                ['spin2', 'final'],
                5
