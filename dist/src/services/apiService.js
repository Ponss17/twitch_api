...
exports.getFollowAge = exports.getClips = exports.createClip = void 0;
...
const createClip = async (channel, token) => {
    ...
    const channelRes = await axios_1.default.get(`https://api.twitch.tv/helix/users?login=${channel}`, { headers });
    if (channelRes.data.data.length === 0) {
        throw { status: 404, message: `El canal ${channel} no existe.` };
    }
...
const getFollowAge = async (channel, user, token) => {
    ...
    const channelRes = await axios_1.default.get(`https://api.twitch.tv/helix/users?login=${channel}`, { headers });
    if (channelRes.data.data.length === 0)
        return `${channel} no existe.`;
    const channelId = channelRes.data.data[0].id;
    const userRes = await axios_1.default.get(`https://api.twitch.tv/helix/users?login=${user}`, { headers });
